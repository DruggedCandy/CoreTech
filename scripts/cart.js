...
class ShoppingCart {
    ...
    updateCartCount() {
        ...
        // Оновлення плаваючого кошика, якщо є
        const floatCount = document.querySelector('.floating-cart .count')
        if (floatCount) {
            floatCount.textContent = count
            if (count > 0) floatCount.style.display = 'inline-flex'
            else floatCount.style.display = 'none'
        }
    }
